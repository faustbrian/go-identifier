...
func TestParserAcceptsLowercaseLaravelStorageAndRejectsMixedCase(t *testing.T) {
	const stored = "01arz3ndektsv4rrffq69g5fav"
	id, err := identifierulid.Parse(stored)
	if err != nil {
		t.Fatalf("Parse(): %v", err)
	}
	if id.StringLower() != stored {
		t.Fatalf("lowercase stored value changed to %q", id.StringLower())
	}
	if id.String() != "01ARZ3NDEKTSV4RRFFQ69G5FAV" {
		t.Fatalf("canonical value = %q", id.String())
	}
	if _, err := identifierulid.Parse("01arz3ndektsv4rrffq69G5fav"); !errors.Is(err, identifier.ErrInvalid) {
		t.Fatalf("mixed-case Parse() error = %v", err)
	}
}
